...
function buildCandidates(input: GreedyPolicyInput, stations: Map<string, StationState>): CandidateEdge[] {
  ...
  for (const edge of input.spec.neighborhood.edges) {
    ...
    if (!from || !to || !from.eligible || !to.eligible) {
      continue;
    }
...
function candidateSort(a: CandidateEdge, b: CandidateEdge): number {
  ...
  if (a.from_station_key !== b.from_station_key) {
    return a.from_station_key.localeCompare(b.from_station_key);
  }
  return a.to_station_key.localeCompare(b.to_station_key);
}

export function runGreedyPolicyV1(
  input: GreedyPolicyInput,
  opts?: { logger?: PolicyLogger }
): GreedyPolicyOutput {
  if (input.policy_version !== "rebal.greedy.v1") {
  ...
  while (bikesBudgetRemaining > 0 && movesRemaining > 0) {
    ...
    for (const candidate of candidates) {
      ...
      if (touched.size + addFrom + addTo > input.spec.effort.max_stations_touched) {
        continue;
      }
      selected = candidate;
      break;
    ...
    if (!selected) {
      logger.info("policy_no_candidate_within_station_budget", {
        system_id: input.system_id,
        decision_bucket_ts: input.decision_bucket_ts,
        max_stations_touched: input.spec.effort.max_stations_touched,
      });
      break;
    }
  ...
  const stationsTouched = Array.from(touched)
    ...
    .map((stationKey) => {
      ...
      if (!station) {
        throw new Error("station_missing");
      }
      return {
        station_key: station.station_key,
        capacity: station.capacity,
  ...
  const bikesMovedTotal = moves.reduce((acc, m) => acc + m.bikes_moved, 0);
  ...
  logger.info("policy_decision_bucket", {
    ...
    bike_budget_remaining: bikesBudgetRemaining,
    move_budget_remaining: movesRemaining,
  });
