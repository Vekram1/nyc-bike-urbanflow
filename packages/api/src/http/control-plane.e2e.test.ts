...
describe("control-plane e2e", () => {
  ...
  it("serves station drawer bundle with sv-bound bounded params", async () => {
    ...
    const invalidTileSchemaRes = await handler(
      new Request(
        `https://example.test/api/stations/STA-001/drawer?v=1&sv=${encodeURIComponent(sv)}&T_bucket=1738872000&range=6h&severity_version=sev.v1&tile_schema=tile.v999`
      )
    );
    expect(invalidTileSchemaRes.status).toBe(400);
    expect(invalidTileSchemaRes.headers.get("Cache-Control")).toBe("no-store");
    ...
    const unsupportedVersionRes = await handler(
      new Request(
        `https://example.test/api/stations/STA-001/drawer?v=2&sv=${encodeURIComponent(sv)}&T_bucket=1738872000&range=6h&severity_version=sev.v1&tile_schema=tile.v1`
      )
    );
  ...
  it("serves station detail and series endpoints with sv-bound params", async () => {
    ...
    const tooManyPointsRes = await handler(
      new Request(
        `https://example.test/api/stations/STA-001/series?sv=${encodeURIComponent(sv)}&from=1738872000&to=1738932000&bucket=60`
      )
    );
    expect(tooManyPointsRes.status).toBe(400);
    expect(tooManyPointsRes.headers.get("Cache-Control")).toBe("no-store");
    ...
    const missingRangeBoundaryRes = await handler(
      new Request(
        `https://example.test/api/stations/STA-001/series?sv=${encodeURIComponent(sv)}&from=1738872000&bucket=300`
      )
    );
    expect(missingRangeBoundaryRes.status).toBe(400);
    expect(missingRangeBoundaryRes.headers.get("Cache-Control")).toBe("no-store");
    const missingRangeBoundaryBody = await missingRangeBoundaryRes.json();
    expect(missingRangeBoundaryBody.error.code).toBe("invalid_range");

    const nonIntegerRangeRes = await handler(
      new Request(
        `https://example.test/api/stations/STA-001/series?sv=${encodeURIComponent(sv)}&from=1738872000.5&to=1738875600&bucket=300`
      )
    );
    expect(nonIntegerRangeRes.status).toBe(400);
    expect(nonIntegerRangeRes.headers.get("Cache-Control")).toBe("no-store");
