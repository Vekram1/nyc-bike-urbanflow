...
describe("createEpisodesTilesRouteHandler", () => {
  ...
  it("returns 405 for non-GET requests", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/tiles/episodes/12/1200/1530.mvt?v=1&sv=abc&T_bucket=1738872000", {
        method: "POST",
      })
    );
  ...
  it("returns 401 when sv token is invalid", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/tiles/episodes/12/1200/1530.mvt?v=1&sv=invalid&T_bucket=1738872000")
    );
    expect(res.status).toBe(401);
    expect(res.headers.get("Cache-Control")).toBe("no-store");
    const body = await res.json();
  ...
  it("returns 401 when sv token is expired", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/tiles/episodes/12/1200/1530.mvt?v=1&sv=expired&T_bucket=1738872000")
    );
    expect(res.status).toBe(401);
    expect(res.headers.get("Cache-Control")).toBe("no-store");
    const body = await res.json();
