...
function parseIsoEpochSeconds(value: string | null): number | null {
...
function deriveEffectiveSnapshotBucket(tBucketEpochS: number | null, snapshot: StationSnapshot[]): number | null {
...
function buildSnapshotIdentity(args: {
  system_id: string;
  view_id: number;
...
export function createStationsRouteHandler(deps: StationsRouteDeps): (request: Request) => Promise<Response> {
  return async (request: Request): Promise<Response> => {
    ...
    if (route.is_snapshot) {
      ...
      const effectiveBucket = deriveEffectiveSnapshotBucket(tBucket, snapshot);
      const { view_snapshot_id, view_snapshot_sha256 } = buildSnapshotIdentity({
        system_id: sv.system_id,
        view_id: sv.view_id,
