...
describe("createStationDrawerRouteHandler", () => {
  ...
  it("returns 403 when sv token is revoked", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/stations/STA-001/drawer?v=1&sv=revoked&T_bucket=1738872000&range=6h")
    );
  ...
  it("returns 401 when sv token is invalid", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/stations/STA-001/drawer?v=1&sv=invalid&T_bucket=1738872000&range=6h")
    );
    expect(res.status).toBe(401);
    expect(res.headers.get("Cache-Control")).toBe("no-store");
    const body = await res.json();
  ...
  it("returns 401 when sv token is expired", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/stations/STA-001/drawer?v=1&sv=expired&T_bucket=1738872000&range=6h")
    );
    expect(res.status).toBe(401);
    expect(res.headers.get("Cache-Control")).toBe("no-store");
    const body = await res.json();
  ...
  it("returns 405 for non-GET requests", async () => {
    ...
    const res = await handler(
      new Request("https://example.test/api/stations/STA-001/drawer?v=1&sv=abc&T_bucket=1738872000&range=6h", {
        method: "POST",
      })
    );
